...
description: "How I diagnosed and fixed a self-inflicted DDoS caused by Bcrypt on a hot path — reducing a 27-node AWS cluster to just two instances, with back-of-the-envelope savings estimated $42k/month in cloud costs."
pubDate: "2026-05-04"
updatedDate: "2026-05-19"
heroImage: "../../assets/how-we-saved-iot-service-200-to-20000-rps/this-is-fine-meme-self-inflicted-ddos-hero-image.webp"
...
import imgFlamegraphA from '../../assets/how-we-saved-iot-service-200-to-20000-rps/pprof-flamegraph-rbac-json-unmarshal-go.webp';
import imgFlamegraphB from '../../assets/how-we-saved-iot-service-200-to-20000-rps/pprof-flamegraph-bcrypt-hotpath-cpu-saturation.webp';
import imgThisIsFine from '../../assets/how-we-saved-iot-service-200-to-20000-rps/this-is-fine-meme-self-inflicted-ddos.webp';
import imgArchDiagram from '../../assets/how-we-saved-iot-service-200-to-20000-rps/request-flow-before-after-bcrypt-cache-singleflight.svg';

{/*
  CONTENT NOTE: "$42k+" is a back-of-the-envelope estimate, NOT
  an actual billing figure. Calculated from ~27 nodes at c6g.16xlarge
  on-demand pricing. Exact costs were never shared with me as a
  contractor. Always frame this as an estimate — never as a confirmed number.
*/}

<TLDRBox>
  A ~27-node AWS cluster estimated to cost north of $42k/month couldn't handle 200 RPS — because every telemetry ping from thousands of connected vehicles was running bcrypt.CompareHashAndPassword on the hot path. Bcrypt is designed to be slow (it's for login flows, not IoT data streams), so we were essentially DDoS-ing ourselves. The fix: cache the validated credential as a SHA-256 hash in Redis with a 30-second TTL, then wrap the Bcrypt call in singleflight to prevent thundering herd on cache expiry. Result: 27 nodes → 2 instances, 200 RPS → 20,000+ RPS.
</TLDRBox>
...
<figure>
  <ZoomImage
    src={imgFlamegraphA}
    alt="Go pprof flamegraph showing RBAC JSON unmarshalling consuming around 20-30% CPU — visible but not catastrophic"
    width={3203}
    height={3471}
  />
  <figcaption>
...
<figure>
  <ZoomImage
    src={imgFlamegraphB}
    alt="Go pprof flamegraph showing bcrypt.CompareHashAndPassword consuming 95%+ of total CPU after adding the IoT device header"
    width={1199}
    height={2023}
  />
  <figcaption>
...
The flame graph was unmistakable. A single function was consuming 95%+ of total CPU time: `bcrypt.CompareHashAndPassword`.
...
<figure>
  <ZoomImage
    src={imgThisIsFine}
    alt="This is Fine meme — a dog sits calmly in a room that is entirely on fire"
    width={716}
    height={349}
  />
  <figcaption>
    Production CPU at 100%. "Oh, we are DDoS-ing ourselves." — <em>This is Fine</em>, KC Green, Gunshow comic (2013). Via <a href="https://knowyourmeme.com/memes/this-is-fine" target="_blank" rel="noopener noreferrer">Know Your Meme</a>.
  </figcaption>
</figure>
...
<figure>
  <ZoomImage
    src={imgArchDiagram}
    alt="Architecture diagram showing request flow before and after the optimization: before, every request hits bcrypt directly; after, a Redis cache and singleflight barrier absorb almost all load"
    width={680}
    height={560}
  />
  <figcaption>
...
| Instance count | ~27 nodes | 2 smaller instances |
| Est. monthly cost | North of $42k/mo* | Drastically reduced |

*Estimated from node count and on-demand pricing for the instance family. Exact billing figures were not shared with me as a contractor.*
